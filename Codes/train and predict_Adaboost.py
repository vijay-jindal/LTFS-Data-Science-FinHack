...
# Here we are using ADABOOST algorithm with RandomForest Classifier as the base classifier
rf=RandomForestClassifier(n_estimators=300,max_features=0.9,max_depth=100,min_samples_split=3,min_samples_leaf=50, n_jobs=-1)

# Making the AdaBoost model with base estimatior as random forest
model = AdaBoostClassifier(base_estimator=rf,n_estimators=50,
                         learning_rate=1)
...
df.to_csv('adaboost_prediction.csv',index=False)
